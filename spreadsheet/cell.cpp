...
Cell::Cell(Sheet& sheet) : impl_(std::make_unique<EmptyImp>()), sheet_(sheet) {}
Cell::~Cell() = default;
...
void Cell::Set(std::string text) {
    ...
    if(text.empty()){
        temp_impl = std::make_unique<EmptyImp>();
...
void Cell::Clear() {
    impl_ = std::make_unique<EmptyImp>();
}
...
Cell::Value Cell::EmptyImp::GetValue() const {return "";}
std::string Cell::EmptyImp::GetText() const {return "";}

Cell::TextImpl::TextImpl(std::string text) : text_(std::move(text)) {}

Cell::Value Cell::TextImpl::GetValue() const {
    if (text_.empty()) {
        throw std::logic_error("empty impl");
